 ...
 */
router.post('/authenticate', authLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
    const { accessToken, metadata }: PrivyAuthRequest = req.body;
    ...
    const result = await privyService.authenticateWithPrivy({
      accessToken,
 ...
 */
router.get('/profile', authenticatePrivy, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/status', async (req: Request, res: Response): Promise<void> => {
  try {
    const status = privyService.getHealthStatus();
